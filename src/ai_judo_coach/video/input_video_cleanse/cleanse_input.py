...
def cleanse_input_video(
    ...
    Optional video output settings are applied independently to both
    cleansing stages. Omitting them preserves the existing local
    FFmpeg behaviour.
    ...
    if video_output_options is None:
        strip_audio(
...
def _get_video_duration_seconds(
    ...
    format_metadata = probe_result.get(
        "format"
    )
    ...
    raw_duration = format_metadata.get(
        "duration"
    )
